...
y[1]<-x1[1]
for(win1:(t-1)){#将a,u的估计值代入时间响应序列函数计算x1拟合序列y
y[w+1]<-(x1[1]-u/a)*exp(-a*w)+u/a
}
cat("x(1)的模拟值：",'\n',y,'\n')
xy<-numeric(length(y))
xy[1]<-y[1]
for(oin2:t){#运用后减运算还原得模型输入序列x0预测序列
xy[o]<-y[o]-y[o-1]
}
cat("x(0)的模拟值：",'\n',xy,'\n','\n')
...
plot(xy,col='blue',type='b',pch=16,xlab='时间序列(始于2000年)',ylab='第二主成分')
...
#a<-c(181.7,154.5,154.1,157.4,256.1,262.8,286.9,307.6,329.8,357.3,403.4,462.7,541.1,614.2,704.7,742.2)
#a<-c(483.4,503.7,527.9,550.6,588,624.5,670.7,734.4,793.152,860.57,954.3721,1063.171)
#a<-c(24.8,25.7,26.5,27.2,27.9,29.7,30.65,31.63,32.4,33.579,34.08,36.24463)
a<-c(9.431174418,11.08166498,13.42571683,13.67367111,17.3241572,18.48019474,20.71151975,22.07472217,25.55958223,26.34468547,29.06276018,32.83015092)
a<-c(21.41306192,20.86062825,19.8389605,19.96813638,18.7949184,18.81116793,19.05015851,20.09993717,19.81374222,20.2181477,19.78696412,21.3441769)
gm11(a,length(a)+4)